...
import {
  getWatchedCount,
  getWatchedWithResourceDetailsPaginated,
  isResourceWatched,
} from './watched';
...
describe('isResourceWatched', () => {
  it('returns false when unauthenticated without querying', async () => {
    vi.mocked(getUser).mockResolvedValue(null as never);

    await expect(isResourceWatched(5, 'movie')).resolves.toBe(false);
    expect(db.select).not.toHaveBeenCalled();
  });

  it('returns true when the item is watched', async () => {
    vi.mocked(db.select).mockReturnValue(chain([{ id: 'item-1' }]));

    await expect(isResourceWatched(5, 'movie')).resolves.toBe(true);
  });

  it('returns false when the item is not watched', async () => {
    vi.mocked(db.select).mockReturnValue(chain([]));

    await expect(isResourceWatched(5, 'tv')).resolves.toBe(false);
  });

  it('returns false on invalid input instead of throwing', async () => {
    await expect(isResourceWatched(0, 'movie')).resolves.toBe(false);
  });
});
...
describe('getWatchedCount', () => {
  it('returns 0 when unauthenticated without querying', async () => {
    ...
    await expect(getWatchedCount('movie')).resolves.toBe(0);
    expect(db.select).not.toHaveBeenCalled();
  });
  ...
  it('returns the count for the resource type', async () => {
    ...
    await expect(getWatchedCount('movie')).resolves.toBe(3);
  });
...
describe('getWatchedWithResourceDetailsPaginated', () => {
  it('rejects an invalid resource type', async () => {
    await expect(getWatchedWithResourceDetailsPaginated('person' as never)).rejects.toThrow(
      'Invalid resource type or page number',
    );
  });
  ...
  it('rejects when unauthenticated', async () => {
    ...
    await expect(getWatchedWithResourceDetailsPaginated('movie')).rejects.toThrow(
      'User not authenticated',
    );
  });

  it('returns an empty page when the watched history is empty', async () => {
    vi.mocked(db.select).mockReturnValue(chain([{ count: 0 }]));

    await expect(getWatchedWithResourceDetailsPaginated('movie')).resolves.toEqual({
      items: [],
      totalItems: 0,
      totalPages: 0,
      currentPage: 1,
  ...
  it('hydrates rows with resource details', async () => {
    ...
    const result = await getWatchedWithResourceDetailsPaginated('movie');
