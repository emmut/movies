...
import { getOrCreateWatchlistListId, getWatchlistListId } from './watchlist-list';

beforeEach(() => {
  vi.clearAllMocks();
});

describe('getWatchlistListId', () => {
  it('returns the list id when the user has a watchlist list', async () => {
    vi.mocked(db.select).mockReturnValue(chain([{ id: 'list-1' }]));

    await expect(getWatchlistListId('user-1')).resolves.toBe('list-1');
  });

  it('returns null when the user has no watchlist list', async () => {
    vi.mocked(db.select).mockReturnValue(chain([]));

    await expect(getWatchlistListId('user-1')).resolves.toBeNull();
  });
});

describe('getOrCreateWatchlistListId', () => {
  it('returns the existing list id without inserting', async () => {
    vi.mocked(db.select).mockReturnValue(chain([{ id: 'list-1' }]));

    await expect(getOrCreateWatchlistListId('user-1')).resolves.toBe('list-1');
  ...
  it('creates the list when missing and returns the new id', async () => {
    ...
    await expect(getOrCreateWatchlistListId('user-1')).resolves.toBe('list-new');
  ...
  it('re-selects the winner when a concurrent create won the race', async () => {
    ...
    await expect(getOrCreateWatchlistListId('user-1')).resolves.toBe('list-winner');
  });
  ...
  it('throws when the list can be neither found nor created', async () => {
    ...
    await expect(getOrCreateWatchlistListId('user-1')).rejects.toThrow(
