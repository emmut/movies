...
type SignInFn = (redirectUrl?: string) => Promise<{ error?: unknown } | void>;
...
type OAuthLoginButtonProps = VariantProps<typeof oauthButtonVariants> &
  React.ComponentProps<'button'> & {
    ...
    icon?: ReactNode;
    redirectUrl?: string;
    /** Called after anonymous sign-in succeeds, to handle navigation */
    onSuccess?: () => void;
    signIn: SignInFn;
  };
...
const defaultIcons: Record<AvailableOAuthProviders, ReactNode> = {
  discord: (
  ...
  anonymous: <UserIcon className="h-4 w-4" />,
};

const defaultTexts: Record<AvailableOAuthProviders, string> = {
  discord: 'Continue with Discord',
  google: 'Continue with Google',
  github: 'Continue with GitHub',
  anonymous: 'Continue as anonymous',
};
...
export function OAuthLoginButton({
  ...
  const displayText = text || defaultTexts[provider];
  const displayIcon = icon !== undefined ? icon : defaultIcons[provider];
  const safeRedirectUrl = getSafeRedirectUrl(redirectUrl);

  async function handleLogin() {
    setIsLoading(true);
    try {
      const result = await signIn(safeRedirectUrl);
