...
**Search follows the same idea.** TMDB's search is a literal title match: no typo tolerance, no control over ranking. Rather than crawl the catalog, the app loads TMDB's daily id exports (every id with its original title and popularity, published precisely so integrators don't crawl) into a `search_index` table with a `pg_trgm` index. On the first results page, TMDB and a bounded GIN-backed similarity query run together. Exact, leading-word, prefix, and substring title-match tiers provide the primary order; reciprocal-rank fusion combines TMDB and index ranks within a tier. Local candidates absent from TMDB are hydrated from the cached details fetchers, capped at three when TMDB already has results. The latency-sensitive command palette remains TMDB-first with a zero-result fuzzy fallback. The exports carry no metadata beyond titles, so TMDB stays the source for everything shown.
